...
class Newsletter extends BaseController
{
    ...
    public function verify($email = null, $hash = null)
    {
        ...
        if (!$this->newsletterModel->check_url($email, $hash)) {
            return redirect()->to(base_url());
        }

        $this->newsletterModel->update($email, $hash, [
            'subs_hash' => '',
            'subs_active' => 1
        ]);
        ...
        return view('view_header', $data)
            . view('view_thankyou_subscribe', $data)
            . view('view_footer', $data);
    }
}
